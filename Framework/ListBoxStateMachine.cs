//---------------------------------------------------------------------
// <copyright file="ListBoxStateMachine.cs" company="Microsoft">
...
namespace CoreInteractionFramework
{
    ...
    {
        /// <summary>
        /// Keep track which contacts have been added to the scroll adapter.
        /// </summary>
        private readonly List<int> scrollAdapterContacts = new List<int>();

        /// <summary>
        /// Used to track how much a contact has changed since it went down on the ListBox.
        /// </summary>
        private readonly Dictionary<int, float> contactChangeDeltaContactIds = new Dictionary<int, float>();

        /// <summary>
        /// Tracks captured contact ContactTargetEvent id.
        /// This is use to track the position a contact is currently at.
        /// </summary>
        private readonly Dictionary<int, ContactTargetEvent> contactTargetEventContactIds =
            new Dictionary<int, ContactTargetEvent>();

        /// <summary>
        /// Tracks captured contact ids to a particular item for which they are "captured".
        /// This is used because <strong>ListBoxStateMachineItem</strong> objects can't actually capture contacts,
        /// but they track which contacts are "logically" captured.
        /// </summary>
        private readonly Dictionary<int, ListBoxStateMachineItem> capturedItemContactIds =
        ...
        /// <summary>
        /// Tests the contact position change to see if its past the threshold of 1/8".
        /// </summary>
        /// <param name="contactId"></param>
        /// <returns></returns>
        private bool DidContactMovePastThreshold(int contactId)
        {
            if (contactChangeDeltaContactIds.ContainsKey(contactId))
            {
                float delta = contactChangeDeltaContactIds[contactId];

                // Check contactChangeDeltaContactIds to see if the value is more than 1/8" in screen space.
